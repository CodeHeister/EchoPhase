...
{
    /// <summary>
    /// Read-only service contract — depends only on <see cref="IReadRepository{TEntity}"/>.
    /// Use when a service never mutates data.
    /// </summary>
    public interface IReadServiceBase<TEntity, TR>
        where TR : IReadRepository<TEntity>
