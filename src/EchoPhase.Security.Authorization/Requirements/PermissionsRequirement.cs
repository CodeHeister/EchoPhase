...
namespace EchoPhase.Security.Authorization.Requirements
{
    public class PermissionsRequirement : IAuthorizationRequirement
    {
            ...
            string serializedPermissions)
        {
            ...
            var res2 = PermissionsBitMask.Deserialize(serializedPermissionsBitmasks);
        ...
        {
            var res = PermissionsBitMask.Serialize(PermissionsBitmasks);

            res.OnFailure(err =>
                throw new InvalidOperationException(err.Value));

            if (!res.TryGetValue(out var result))
                throw new NullReferenceException($"Missing result from {typeof(PermissionsBitMask).Name}.");
