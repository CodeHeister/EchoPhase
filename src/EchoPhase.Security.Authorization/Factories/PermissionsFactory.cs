...
namespace EchoPhase.Security.Authorization.Factories
{
    ...
    {
        private readonly IPermissionsBitMask _permissionsBitmaskService;

        public PermissionsFactory(IPermissionsBitMask permissionsBitmaskService)
        {
        ...
        public PermissionsRequirement Requirement(params (Resources resource, string[] perms)[] resourcePerms)
        {
            ...
            foreach (var (resource, perms) in resourcePerms)
            {
                ...
                var current = dict.GetValueOrDefault(resource) ?? BitMaskBase.Empty;
