...
{
    public class PermissionsBitMaskService : BitMaskServiceBase, IPermissionsBitMaskService
    {
        ...
        public PermissionsBitMaskService()
            : base(Permissions.AsEnumerable().Order()) { }

        public IServiceResult<IReadOnlyDictionary<Resources, BitArray>> Encode(IReadOnlyDictionary<Resources, string[]> dict)
        {
            if (dict is { Count: 0 })
            {
            ...
            foreach (var (resource, perms) in dict)
            {
                ...
                var current = masked.GetValueOrDefault(resource) ?? BitMaskServiceBase.Empty;
