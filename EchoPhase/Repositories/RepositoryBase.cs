...
{
    public abstract class RepositoryBase<TR, TO> : IRepositoryBase<TR, TO>
		where TR : RepositoryBase<TR, TO>
		where TO : class, new()
		{
		...
		)
		{
			...
			{
				extraFilters(ApplySearchOptions<T,TOptions>(query, options), options);
			}
